...
def SSL(M = 1, K = 5, alpha = 100, ratio = 50):
  ...
  ratio = 0.4; #assuming 1% is labeled
  ...
  same_class_probability = 60;
  diff_class_probability = 30;

  stochastic_graph = np.zeros((nodes, nodes))
  for curr_node in range(nodes):
    for other_node in range(nodes):
      random_int = rand.randint(0,100);
      prob = diff_class_probability;
      if X_labels[curr_node] == X_labels[other_node]:
        prob = same_class_probability;
      if random_int <= prob:
        stochastic_graph[curr_node][other_node] = prob/100;
        stochastic_graph[other_node][curr_node] = prob/100; #not sure what value to use for connected edges


  stochastic_degrees = np.zeros(nodes);
  for i in range(nodes):
    stochastic_degrees[i] = np.count_nonzero(stochastic_graph[i]);


  Dmatrix = np.zeros((nodes, nodes))
  for i in range(nodes):
    Dmatrix[i][i] = np.sum(stochastic_graph[i])

  Dinv = np.linalg.inv(Dmatrix)
  S = Dinv@stochastic_graph;

  labeled_degrees = np.zeros(labeled_amnt)
  for index in range(labeled_amnt):
    labeled_index = random_index[index]
    labeled_degrees[index] = stochastic_degrees[labeled_index]
